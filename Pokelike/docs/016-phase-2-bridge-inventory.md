...
Scout Report, Contract Offer, Squad Registration, and City Stamp are already behaviorally football-native but still include legacy compatibility anchors. P2-011 adds Scout Report wrapper classes while keeping the old IDs for event handlers. P2-012 adds Contract Offer state classes while keeping the existing overlay base. P2-013 adds Squad Registration layout classes while preserving swap IDs for routing and keyboard shortcuts. P2-014 adds City Stamp selector classes while preserving badge IDs for Classic and shortcuts. Phase 2 should improve player-facing structure first and only remove selector aliases after tests prove handlers remain stable.
