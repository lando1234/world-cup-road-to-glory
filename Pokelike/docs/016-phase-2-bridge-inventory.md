...
| `catch-screen` container for Scout Report | Retire player-facing structure gradually | Recruitment UI | `index.html`, `js/game.js`, `css/style.css` | P2-011 |
...
Scout Report, Contract Offer, Squad Registration, and City Stamp are already behaviorally football-native but still ride legacy container names. Phase 2 should improve player-facing structure first and only remove selector aliases after tests prove handlers remain stable.
