...
await runTest("scout pools define slice, late, and finale stage bands", () => {
  ...
  const finale = context.window.DomainScout.getBandForMap(6);
  ...
  assert(finale.bandId === "finale", "mapIndex 6 should use finale scout band");
  ...
  assert(finale.mapMin === 6 && finale.mapMax === 7, "finale scout band should cover maps 6-7");
  ...
  const map6Report = context.window.DomainScout.buildSliceReport(6, {}, { node: { layer: 1, type: "catch" } });
  assert(map6Report.profileIds.length === 3, "map 6 scout report should return three choices");
  assert(map6Report.profileIds.every(id => finale.profileIds.includes(id)), "map 6 scout choices must come from finale band");
});
