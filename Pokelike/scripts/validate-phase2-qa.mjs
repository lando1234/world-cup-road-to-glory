...
runTest("P2-024 eight-host-city expansion decision is prepare-only", () => {
  ...
  assert(featuresSource.includes("maxMapIndex: 7"), "runtime map cap must be 7 after Phase 3 enable");
});
...
runTest("P2-026 scout pool expansion guard validates late bands without runtime load", () => {
  ...
  assert(slicePools.bands.some(band => band.bandId === "finale"), "runtime scout pools should include merged finale band after Phase 3");
...
runTest("P2-027 album page expansion guard prepares deferred pages", () => {
  ...
  const legendsPage = sliceLayout.pages.find(page => page.pageId === "legends");
  if (knockoutPage && legendsPage) {
    assert(knockoutPage.hiddenUntil === "knockout_enable", "runtime knockout page should stay hidden until knockout_enable");
    assert(legendsPage.hiddenUntil === "legends_enable", "runtime legends page should stay hidden until legends_enable");
  } else {
